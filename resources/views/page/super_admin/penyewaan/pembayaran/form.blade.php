  ...
  <div class="modal-dialog modal-lg modal-dialog-scrollable" role="document">
    <div class="modal-content">
      ...
      <div class="row mt-2" id="row_reminder">
        <h4 class="text" style="color: black;">Preview Pesan Reminder:</h4>
        @foreach($reminder as $rem)
        <div id="reminderText">
          @foreach($reminder as $index => $rem)
          <p id="reminderTextContent">
           {!! nl2br(e(str_replace(
            ['$name', '$kamar', '$cabang', '$mulai','$selesai','$nomor_admin'],
            [$dt->name, $dt->nomor_kamar, $dt->nama_cabang, tanggal_indonesia($dt->tanggal_penyewaan), tanggal_indonesia($dt->tanggal_selesai),$dt->ponsel_operator],
            $rem->pesan_reminder
            ))) !!}
          </p>
          @endforeach
        ...
        <div class="col-12">
         <div class="table-responsive">
          ...
          <tbody>
            ...
            <tr>
              <td>
                <input type="checkbox" more_bulan="{{bulan_to_nama($tbb->bulan_pembayaran)}} - {{$tbb->tahun_pembayaran}}" more_harga="{{$tbb->harga_pembayaran}}" more_penyewaan="{{date('d', strtotime($dt->tanggal_penyewaan))}}" value="{{bulan_to_nama($tbb->bulan_pembayaran)}}{{$tbb->bulan_pembayaran}}" class="form-check-input pilih_tagihan" name="pilih_tagihan[]">
...
<div class="modal-footer">
   <!--  <button type="button" class="btn" data-bs-dismiss="modal">
    ...
    <button class="btn btn-primary w-100 submit">
      <span id="label_submit_bayar">PEMBAYARAN</span>
    </button>
  </div>
</form>
</div>
