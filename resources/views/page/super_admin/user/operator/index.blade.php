      ...
      <div class="card">
        <div class="card-header">
          <h5 class="card-title mb-0">
            Data User Operator {!! $label_cabang ? '<strong>(Cabang : ' . $label_cabang->nama_cabang . ')</strong>' : '' !!}
        ...
        <div class="card-body">
          <div class="table-responsive">
            <table class="table table-striped dt-responseive nowrap" id="table_operator" style="width: 100%;">
              <thead>
                <tr>
  ...
  <div class="modal fade text-left" data-bs-backdrop="static" id="modal_form_operator" tabindex="-1" role="dialog"
          ...
          <div class="col-lg-12">
            ...
            <div class="col-lg-12 mb-2">
              <div class="form-group">
                ...
                <input type="text" class="form-control input_view" autocomplete="off" id="name" name="name">
            ...
            <div class="col-lg-12 mb-2">
              <div class="form-group">
                ...
                <input type="email" autocomplete="off" class="form-control input_view" id="email" name="email">
...
<script type="text/javascript">
    ...
    $('#table_operator').DataTable({
      ...
      ajax: {
        ...
        error: function (jqXHR, textStatus, errorThrown) {
          $('#table_operator').DataTable().ajax.reload();
        }
      },
      columns: [
  ...
  $("#id_cabang").select2({
    ...
    dropdownParent: $("#modal_form_operator")
  });
  $("#status_user").select2({
  ...
  $(document).ready(function() {
    $(".new").click(function() {
      ...
      setTimeout(function() {
        ...
        $("#operatorForm input").removeClass("is-invalid");
        $("#operatorForm select").removeClass("is-invalid");
        $(".modal-title").html('<i class="bx bx-plus"></i> Form Tambah Operator');
        // $(".view_akun").html('');
        // $(".img_view").css('display','none');
        $("#id_cabang").val(null).trigger('change');
        $("#status_user").val(null).trigger('change');
        $("#modal_form_operator").modal('show');
        ajaxUrl = "{{route('save.operator')}}";
      }, 300);
    });
  });
  ...
  // });
  $(function () {
    $('#operatorForm').submit(function (e) {
      e.preventDefault();
      ...
      $.ajax({
        ...
        success: function (response) {
          $('#operatorForm').data('submitted', false);
          $("#loading").hide();
          if (response.status == 'true') {
            $("#operatorForm")[0].reset();
            $('#modal_form_operator').modal('hide');
  ...
  function get_edit(userID) {
    $.ajax({
      type: "GET",
      url: "{{url('page/user/operator/get_edit')}}"+"/"+userID,
      success: function(response) {
        if (response) {
          $("#loading").hide();
          $.each(response, function(key, value) {
            ...
            // $('.img_view').attr("src","{{asset('foto_profil')}}/"+value.foto);
            // if (value.foto != null) {
            //   $(".img_view").css('display','block');
            // }else{
            //   $(".img_view").css('display','none');
  ...
  $(document).on('click','.edit',function() {
    ...
    $(".invalid-feedback").children("strong").text("");
    $("#operatorForm input").removeClass("is-invalid");
    $(".modal-title").html('<i class="bx bx-edit"></i> Form Ubah Operator');
    $("#modal_form_operator").modal('show');
    // $(".view_akun").html('');
    // $(".img_view").css('display','none');
    $("#id_cabang").val(null).trigger('change');
    $("#status_user").val(null).trigger('change');
    ajaxUrl = "{{route('update.operator')}}";
    if (userID) {
      get_edit(userID);
    }
  });
  $(document).on('click', '.delete', function (event) {
    ...
    Swal.fire({
      ...
      text: 'Data User Operator akan dihapus secara Permanent!',
      icon: 'warning',
      type: 'warning',
      showCancelButton: !0,
      ...
      if (result.isConfirmed) {
        ...
        $.ajax({
          ...
          success:function(response)
          {
            ...
            if (response.status == 'true') {
              setTimeout(function(){
                showToast('bg-success','Operator Dihapus',response.message);
                $('#table_operator').DataTable().ajax.reload();         
              }, 50);
            }else{
              showToast('bg-danger','Operator Error',response.message);
            }
          },
          error: function(response) {
            $("#loading").hide();
